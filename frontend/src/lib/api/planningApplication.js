...
export async function suggestArgument(projectId, { file, text, documentType, documentTitle, userNotes, relevantTrackIds, conversation, customPrompt, preview = false }) {
  const qs = preview ? '?preview=true' : '';
  if (file) {
    ...
    const res = await authFetch(`${BASE}/projects/${projectId}/suggest-argument${qs}`, { method: 'POST', body: fd });
    if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error || 'Failed to generate suggestion'); }
    return res.json();
  } else {
    const res = await authFetch(`${BASE}/projects/${projectId}/suggest-argument${qs}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, document_type: documentType, document_title: documentTitle || '', user_notes: userNotes, relevant_track_ids: relevantTrackIds ?? [], conversation: conversation ?? [], custom_prompt: customPrompt ?? null })
    });
    if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error(e.error || 'Failed to generate suggestion'); }
    return res.json();
  }
...
export async function getSuggestTemplate(projectId) {
  const res = await authFetch(`${BASE}/projects/${projectId}/suggest-template`);
...
export async function deleteSuggestTemplate(projectId) {
  const res = await authFetch(`${BASE}/projects/${projectId}/suggest-template`, { method: 'DELETE' });
