<script>
  ...
  function submit() {
    ...
    const s = specificIssue.trim();
    if (!d && !s) { error = 'Enter at least a discipline or a specific issue'; return; }
    dispatch('add', { discipline: d || null, label: s || null });
  }
...
<div class="modal-backdrop" on:click|self={close}>
  <div class="modal-content" style="max-width:440px">
    ...
    <div class="form-body">
      ...
      <div class="form-group">
        <label for="specific-issue">Specific Issue <span class="optional">optional</span></label>
        <input id="specific-issue" type="text" bind:value={specificIssue} placeholder="e.g. Listed Building at Risk…" on:keydown={(e) => e.key === 'Enter' && submit()} />
      </div>
