<script>
  ...
  import { getRenewables, toggleRenewableDismissed } from '$lib/services/projectmap/projectMapApi.js';
  ...
  async function handleDismissToggle(item) {
    try {
      ...
      // Update local data
      renewablesData = renewablesData.map(r =>
        r.id === item.id ? { ...r, dismissed: newDismissed } : r
      );

      console.log(`✅ Toggled renewable ${item.id} dismissed to ${newDismissed}`);
    } catch (error) {
  ...
  onMount(async () => {
    try {
      loading = true;
      const result = await getRenewables();
      // Extract properties from GeoJSON features
      renewablesData = result.features.map(f => f.properties);
      console.log(`✅ Loaded ${renewablesData.length} renewables for table`);
    } catch (error) {
...
<div class="renewables-tab">
  ...
  {:else if renewablesData && renewablesData.length > 0}
    <div class="table-header">
      <h3>Renewables</h3>
      <span class="record-count">{renewablesData.length} {renewablesData.length === 1 ? 'record' : 'records'}</span>
    </div>
