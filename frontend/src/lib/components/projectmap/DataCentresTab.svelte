<script>
  ...
  $: displayData = (() => {
    if (!filterResults) return dataCentresData;
    if (showOnlyRelevant) return dataCentresData.filter(item => filterResults.get(item.id)?.relevant);
  ...
  async function handleDismissToggle(item) {
    try {
      ...
      dataCentresData = dataCentresData.map(dc => dc.id === item.id ? { ...dc, dismissed: newDismissed } : dc);
    } catch (error) {
  ...
  async function handleApplyFilter() {
    ...
    filterError = '';
    try {
      const { results } = await applyScraperFilter('datacentres');
  ...
  async function savePrompt() {
    ...
    try {
      await updateScraperFilterPrompt('datacentres', draftPrompt.trim());
  ...
  onMount(async () => {
    try {
      loading = true;
      const [result, prompts] = await Promise.all([getDataCentres(), getScraperFilterPrompts()]);
      dataCentresData = result.features.map(f => f.properties);
      const row = prompts.find(p => p.category === 'datacentres');
    ...
    <div class="table-header">
      <div class="header-top">
        ...
        <span class="record-count">
          {#if filterResults}
            {relevantCount} relevant / {dataCentresData.length} total
          {:else}
            {dataCentresData.length} {dataCentresData.length === 1 ? 'record' : 'records'}
          {/if}
        </span>
      </div>
...
<style>
  ...
  .btn-filter { background: var(--color-primary-500); color: white; border-color: var(--color-primary-500); }
  .btn-filter:hover:not(:disabled) { background: var(--color-violet-700); }
  .btn-filter:disabled { opacity: 0.6; cursor: not-allowed; }

  .btn-clear { background: var(--color-slate-100); color: var(--color-slate-600); border-color: var(--color-slate-200); }
  .btn-clear:hover { background: var(--color-slate-200); }

  .btn-edit-prompt { background: white; color: var(--color-slate-500); border-color: var(--color-slate-200); }
  ...
  .filter-error { margin: 0; font-size: 0.8rem; color: var(--color-red-800); }
  ...
  .prompt-editor-actions { display: flex; gap: 0.5rem; }
  ...
  .data-table tbody tr.dismissed { background-color: var(--color-slate-100); opacity: 0.6; }
  .data-table tbody tr.dismissed:hover { background-color: var(--color-slate-200); }
  ...
  .data-table tbody tr.not-relevant { opacity: 0.35; }

  .data-table td { padding: 0.4rem 0.75rem; color: var(--color-slate-700); }
  ...
  .cell-address span { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: help; }
