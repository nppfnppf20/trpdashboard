...
async function checkWorldHeritage() {
  ...
  try {
    console.log('🔍 Checking for World Heritage Site tables...\n');
    
    // Search for tables containing 'world' or 'heritage'
    const tables = await pool.query(`
      SELECT table_name 
      FROM information_schema.tables 
      WHERE table_schema = 'public' 
      AND (LOWER(table_name) LIKE '%world%' OR LOWER(table_name) LIKE '%heritage%')
      ORDER BY table_name;
    `);
    
    console.log('📊 Tables matching "world" or "heritage":');
    ...
    // Check the world heritage site table
    const targetTable = 'world_heritage_site';
    ...
    if (columns.rows.length === 0) {
      ...
      const alternatives = ['World_heritage_site', 'world heritage site', 'World Heritage Site', 'WorldHeritageSite'];
...
checkWorldHeritage().catch(console.error);
