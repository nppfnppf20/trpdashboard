...
async function checkParksGardens() {
  ...
  try {
    console.log('🔍 Checking for Parks and Gardens tables...\n');
    
    // Search for tables containing 'park' or 'garden'
    const tables = await pool.query(`
      SELECT table_name 
      FROM information_schema.tables 
      WHERE table_schema = 'public' 
      AND (LOWER(table_name) LIKE '%park%' OR LOWER(table_name) LIKE '%garden%')
      ORDER BY table_name;
    `);
    
    console.log('📊 Tables matching "park" or "garden":');
    ...
    // Check the registered parks and gardens table
    const targetTable = 'registered_parks_gardens';
    ...
    if (columns.rows.length === 0) {
      ...
      const alternatives = ['Registered_parks_gardens', 'registered parks and gardens', 'Registered parks and gardens'];
...
checkParksGardens().catch(console.error);
