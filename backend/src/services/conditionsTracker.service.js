...
const CANDIDATE_SYSTEM_PROMPT = `You are a planning consultant assistant maintaining a planning conditions discharge tracker. The user will provide:

1. SOURCE MATERIAL — typically an email trail or a typed note describing recent progress on discharging planning conditions.
2. Every CONDITION currently tracked for this project, each with its number, title, current status, and a one-line note of where things currently stand.

Nothing has been pre-selected. Work out for yourself which condition(s), if any, the source material is actually about, then write the next dated progress entry for each one.

Relevance — decide per condition:
- Return an <ITEM> only for conditions the source material genuinely mentions or is clearly about. Most conditions in the list will NOT be relevant — do not pad the list to cover more conditions than the material actually supports. Returning only one or two <ITEM>s, or none at all, is the normal and expected outcome.
- If the source material doesn't relate to any of the listed conditions, return exactly:
<NONE/>
...
Return your response using EXACTLY this XML structure — one <ITEM> block per relevant condition, nothing before the first <ITEM> and nothing after the last </ITEM>:
...
export async function suggestAdvancementCandidates(fullText, conditions) {
  const conditionBlocks = conditions.map(c => `CONDITION (id: ${c.id})
Number: ${c.condition_number || 'n/a'}
Title: ${c.title}
Current status: ${c.status || 'n/a'}
Latest progress: ${c.latest_summary || 'no progress entries yet'}`).join('\n\n');
