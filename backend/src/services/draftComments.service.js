...
export async function listComments(projectId, draftKind, draftTypeId) {
  const { rows } = await pool.query(
    `SELECT c.id, c.project_id, c.draft_kind, c.draft_type_id, c.paragraph_id,
            c.quoted_text, c.body, c.resolved, c.created_at, c.updated_at,
...
export async function createComment(authorId, { projectId, draftKind, draftTypeId, paragraphId, quotedText, body }) {
  const { rows } = await pool.query(
    `INSERT INTO public.draft_paragraph_comments
       (project_id, draft_kind, draft_type_id, paragraph_id, quoted_text, body, author_id)
    ...
    [projectId, draftKind, draftTypeId, paragraphId, quotedText, body, authorId]
  );
  return rows[0];
}
