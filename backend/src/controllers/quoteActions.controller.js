...
export async function getActionsForProject(req, res) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT qa.id, qa.quote_id, qa.action_date::text, qa.summary, qa.full_text, qa.source_type
 ...
 * Body: { action_date, full_text, source_type, items: [{ quote_id, summary }] }
 */
export async function createActions(req, res) {
  const { projectId } = req.params;
  const { action_date, full_text, source_type, items } = req.body;
    ...
    for (const item of items) {
      const { rows } = await client.query(
        `INSERT INTO admin_console.quote_actions
           (quote_id, action_date, summary, full_text, source_type)
         SELECT $1, $2, $3, $4, $5
         WHERE EXISTS (
           SELECT 1 FROM admin_console.quotes q
           WHERE q.id = $1 AND q.project_id = $6
         )
         RETURNING id, quote_id, action_date::text, summary, full_text, source_type`,
        [
          item.quote_id,
          action_date,
          item.summary.trim(),
 ...
 * Body: { full_text, items: [{ quote_id, user_summary }] }
 */
export async function suggestActions(req, res) {
  ...
  try {
    ...
    const [{ rows: quotes }, { rows: actions }] = await Promise.all([
      pool.query(
        `SELECT q.id, q.discipline, q.work_status, so.organisation AS surveyor_organisation,
                COALESCE(
                  json_agg(qli.item ORDER BY qli.created_at)
                    FILTER (WHERE qli.id IS NOT NULL AND COALESCE(qli.is_instructed, false)),
                  '[]'
                ) AS line_items
