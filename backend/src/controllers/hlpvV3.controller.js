/**
 * HLPV v3 Controller
 * Brand new High-Level Planning View document type, built by copying the
 * stage1_review_v3 architecture exactly: self-contained master prompt (no
 * guiding brief, no style template import), real project-data tokens
 * substituted directly, single LLM call, own draft type / draft storage.
 */
...
After completing these checks, output only the completed High-Level Planning View in clean HTML.`;
...
export async function generateHlpvV3(req, res) {
  ...
  try {
    ...
    const lpaText = Array.isArray(lpaArr) ? lpaArr.join(', ') : (lpaArr || null);
    const subSectorsText = Array.isArray(project.sub_sectors) ? project.sub_sectors.join(', ') : project.sub_sectors;
    const siteAndProposalLines = [
      project.project_name && `Site name: ${project.project_name}`,
      project.address && `Address: ${project.address}`,
      project.development_type && `Development type: ${project.development_type}`,
      lpaText && `Local Planning Authority: ${lpaText}`,
      subSectorsText && `Sub-sectors: ${subSectorsText}`,
    ].filter(Boolean);
    const siteAndProposalInformation = siteAndProposalLines.length ? siteAndProposalLines.join('\n') : '(not provided)';

    // ── 4. Planning Context ─ local/neighbourhood/national policies; emerging has no
    // schema home (project_policies.policy_type CHECK constraint excludes it) ────────
    ...
    const neighbourhoodPolicies = formatPolicies(policyRows.filter(p => p.policy_type === 'neighbourhood'));
    const nationalPolicies = formatPolicies(policyRows.filter(p => p.policy_type === 'national'));
    const planningContext = policyRows.length
      ? [
          `Local Policy:\n${localPolicies}`,
          `Neighbourhood Plan Policy:\n${neighbourhoodPolicies}`,
          `National Policy:\n${nationalPolicies}`,
          'Emerging Policy: (not provided)',
        ].join('\n\n')
      : '(not provided)';

    // ── 5. Drafting Issue Notes ─ same mechanism as stage1_review_v3 (migration 137) ───
    ...
    // ── 6. Planning history ─ same source/format as Stage 1 Review ───────────────
    ...
    // ── 7. Load prompt template from DB; fall back to default ───────────────
    const { rows: promptRows } = await pool.query(
      `SELECT prompt_text FROM admin_console.llm_prompts WHERE prompt_key = $1`,
      [promptKey]
    );
    const promptTemplate = promptRows[0]?.prompt_text ?? DEFAULT_HLPV_V3_TEMPLATE;

    // ── 8. Substitute variables ────────────────────────────────────────
    // DESIGNATIONS_AND_CONSTRAINTS / FIGURES_AND_CAPTIONS / OTHER_PROJECT_DOCUMENTS /
    // CORRESPONDENCE_DETAILS / ADDITIONAL_DRAFTING_INSTRUCTIONS have no backing data
    // source anywhere in the system today (see migration 138 for why each is kept as
    // a token rather than stripped). Always "(not provided)" so the prompt's own
    // fallback instructions take over; wire up a real source later without a rewrite.
    const userPrompt = promptTemplate
      .replace(/\{\{BRIEFING_NOTES\}\}/g, briefingPlain)
      .replace(/\{\{DRAFTING_ISSUE_NOTES\}\}/g, draftingIssueNotes)
      .replace(/\{\{SITE_AND_PROPOSAL_INFORMATION\}\}/g, siteAndProposalInformation)
      .replace(/\{\{PLANNING_CONTEXT\}\}/g, planningContext)
      .replace(/\{\{DESIGNATIONS_AND_CONSTRAINTS\}\}/g, '(not provided)')
      .replace(/\{\{PLANNING_HISTORY\}\}/g, planningHistoryBlock)
      .replace(/\{\{FIGURES_AND_CAPTIONS\}\}/g, '(not provided)')
      .replace(/\{\{OTHER_PROJECT_DOCUMENTS\}\}/g, '(not provided)')
      .replace(/\{\{CORRESPONDENCE_DETAILS\}\}/g, '(not provided)')
    ...
    if (typeRows.length) {
      ...
      const { rows: draftRows } = await pool.query(
        `INSERT INTO planning_applications.drafts
         ...
         RETURNING *`,
        [projectId, draftTypeId, bodyHtml]
      );
      return res.json(draftRows[0]);
    }

    res.json({ html: bodyHtml });
  } catch (err) {
    console.error('hlpvV3.generate error:', err);
