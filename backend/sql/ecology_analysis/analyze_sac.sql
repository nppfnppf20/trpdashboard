-- PostgreSQL function for Special Areas of Conservation (SAC) spatial analysis
-- Analyzes SACs (habitat conservation sites under EU Habitats Directive) relative to a drawn polygon
...
DROP FUNCTION IF EXISTS analyze_sac(TEXT);

CREATE OR REPLACE FUNCTION analyze_sac(polygon_geojson TEXT)
RETURNS TABLE (
  id INTEGER,
  name TEXT,
  code TEXT,
...
sac_sites AS (
  SELECT
    ...
    s.name AS name,
    s.entity AS code
  FROM public."special-area-of-conservation" s
