-- Stage 1 Planning Appraisal v3: replace the prompt authored in migration 136
-- with the user's own hand-authored master prompt (drafted externally), reviewed
-- and mapped onto this system's real variables.
--
-- Variable mapping applied during incorporation:
--   {{PROJECT_BRIEFING_NOTE}} -- renamed to {{BRIEFING_NOTES}} (real token, unchanged)
--   {{HIGH_LEVEL_PLANNING_REVIEW}} -- kept, real token, unchanged
--   {{PLANNING_HISTORY}} -- kept, real token, unchanged
--   {{LOCAL_POLICIES}} / {{NATIONAL_POLICIES}} -- kept, real tokens, unchanged
--
--   {{PROJECT_ISSUE_NOTES}} -- renamed to {{DRAFTING_ISSUE_NOTES}} and given a
--     real backing source: admin_console.drafting_issues (+ drafting_issue_policy_relevance),
--     the same mechanism Planning Statement v3 uses for its Planning Assessment
--     section (project-scoped, surfaced in the same workspace's "Drafting Issues"
--     tab) -- not the older admin_console.project_issue_tracks /
--     planning_applications.issue_notes pair. Confirmed by direct instruction:
--     this should mirror Planning Statement's mechanism, not the legacy one.
--
--   Next Steps / Strategy -- confirmed by direct instruction to come from the
--     Project Briefing Note ONLY, not from the Drafting Issue Notes. Every
--     reference to Next Steps / Strategy sourcing has been rewritten accordingly
--     throughout (Section 7.1, individual issues, Section 8, and the Final
--     Quality Checks), since neither admin_console.drafting_issues nor the
--     older project_issue_tracks/issue_notes pair has a dedicated "next steps"
--     field -- specialist_report on drafting_issues records evidence about an
--     EXISTING report, not a recommended future action, so it was not a fit.
--
--   {{OTHER_PROJECT_DOCUMENTS}}, {{EMERGING_POLICIES}}, {{REPORTS_REQUIRED_FOR_APPLICATION}},
--     {{ADDITIONAL_DRAFTING_INSTRUCTIONS}} -- kept as tokens (not stripped), since
--     each is a real structural part of the prompt (e.g. Reports Required for the
--     Application is a whole document section) or a widely-referenced source
--     throughout the rules. No backing data source exists anywhere in the system
--     today (project_policies.policy_type has a DB CHECK constraint limited to
--     local/national/neighbourhood -- "emerging" isn't just unpopulated, it's
--     disallowed; there is no upload slot for other documents, application
--     reports, or drafting instructions). The controller always substitutes
--     "(not provided)" for these four, so the prompt's own built-in fallback
--     placeholders take over -- a real source can be wired in later without
--     another prompt rewrite.
--
-- Numbered headings and sub-numbering (1. Summary, 6.1 Adopted Policy, 7.2/7.3
-- issue numbers) removed throughout per explicit instruction -- this document
-- follows the same unnumbered-heading convention as every other prompt in this
-- codebase. Several "Heading:"/"Begin with:"/"Start directly with:" lines in
-- the source document had lost their actual HTML content (inline code
-- formatting stripped when the source was converted to plain text before
-- pasting) -- sensible values inferred and restored, matching the equivalent
-- placeholder wording already used elsewhere in the same document where a
-- literal example survived the conversion (e.g. the permitted-placeholders
-- list in the Missing Information section).
--
-- Scoped to Stage 1 Planning Appraisal v3 only. stage1_review (v1) and
-- stage1_review_v2 are untouched.

UPDATE admin_console.llm_prompts
SET prompt_text = $s1v3$## Role and Objective
...
After completing these checks, output only the final Stage 1 Planning Appraisal in clean HTML.$s1v3$,
    updated_at = NOW()
WHERE prompt_key = 'stage1_review_v3';
