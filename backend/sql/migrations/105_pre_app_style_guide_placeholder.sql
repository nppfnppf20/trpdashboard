-- Add {{STYLE_GUIDE}} placeholder to the pre_application_request prompt, so the
-- guiding brief's style example is carried inline in the editable prompt text
-- (like every other {{}} variable) instead of being auto-appended in code.
-- Scoped to pre_application_request only — other appeal draft types generated via
-- generateAppealDraftFromPrompt() have no {{STYLE_GUIDE}} token and keep the
-- existing auto-appended "## Example Document" behaviour, unchanged.
