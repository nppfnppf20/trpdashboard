-- PostgreSQL function for National Parks spatial analysis
-- Analyzes National Parks relative to a drawn polygon
...
DROP FUNCTION IF EXISTS analyze_national_parks(TEXT);
...
-- National Parks polygons
national_parks_polys AS (
  SELECT
    ST_MakeValid(
      CASE WHEN ST_SRID(np.geom) = 27700 THEN np.geom ELSE ST_Transform(np.geom, 27700) END
    ) AS geom,
    np."OBJECTID"::INTEGER AS id,
    np."NAME" AS name
  FROM public."National parks England" np
  WHERE np.geom IS NOT NULL
),
...
with_buffers AS (
  SELECT
    ...
    -- Calculate direction (azimuth from site to National Park)
